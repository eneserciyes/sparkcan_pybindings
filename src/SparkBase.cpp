...
#include <unordered_map>   // registry
#include <atomic>          // std::atomic
#include <system_error>    // nicer errno -> exception
#include <iostream>

int  SparkBase::soc_ = -1;
std::atomic<bool> SparkBase::rxRunning_{false};
std::thread SparkBase::rxThread_;
std::unordered_map<uint8_t, SparkBase*> SparkBase::registry_;
std::mutex SparkBase::registryMtx_;
...
: interfaceName_(interfaceName), deviceId_(deviceId)
{
  ...
  // Start (or reuse) the shared RX thread, which opens the CAN socket once:
  StartRxThread(interfaceName_);

  // Register this motor so the thread can dispatch its frames
  {
    std::lock_guard<std::mutex> lk(registryMtx_);
    registry_[deviceId_] = this;
  }
...
SparkBase::~SparkBase()
{
  ...
  std::lock_guard<std::mutex> lk(registryMtx_);
  registry_.erase(deviceId_);
  if (registry_.empty()) StopRxThread();   // last motor → shut the reader down

}

/* ────────────────────────────────────────────────────────────── *
 *  Shared RX-thread lifecycle helpers                           *
 * ────────────────────────────────────────────────────────────── */
void SparkBase::StartRxThread(const std::string& ifname)
{
  if (rxRunning_) return;                    // already running

  /* -------- open CAN socket ONCE -------- */
  soc_ = socket(PF_CAN, SOCK_RAW, CAN_RAW);
  if (soc_ < 0) throw std::system_error(errno, std::generic_category(),"socket");
  ...
  if (bind(soc_, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0)
      throw std::system_error(errno, std::generic_category(),"bind");

  /* non-blocking */
  fcntl(soc_, F_SETFL, fcntl(soc_, F_GETFL, 0) | O_NONBLOCK);

  rxRunning_ = true;
  rxThread_  = std::thread(&SparkBase::RxLoop);
}

void SparkBase::StopRxThread()
{
  if (!rxRunning_) return;
  rxRunning_ = false;
  if (rxThread_.joinable()) rxThread_.join();
  if (soc_ != -1) { close(soc_); soc_ = -1; }
}

void SparkBase::RxLoop()
{
  can_frame frame{};
  while (rxRunning_) {
    ssize_t n = read(soc_, &frame, sizeof(frame));
    if (n <= 0) { std::this_thread::sleep_for(std::chrono::milliseconds(1)); continue; }

    uint8_t id = static_cast<uint8_t>(frame.can_id & 0x3F);   // low 6 bits = deviceId
    SparkBase* motor = nullptr;
    {
      std::lock_guard<std::mutex> lk(registryMtx_);
      auto it = registry_.find(id);
      if (it != registry_.end()) motor = it->second;
    }
    if (motor) motor->HandlePeriodicFrame(frame);
  }
}
...
uint32_t SparkBase::CreateArbId(APICommand cmd) const
{
  ...
  // deviceInfo": { "deviceType": "MotorController", "deviceTypeNumber": 2, "manufacturer": "REV", "manufacturerNumber": 5 }
...
/* ────────────────────────────────────────────────────────────── *
 *  Per-motor frame processing (moved from old ReadPeriodic…)     *
 * ────────────────────────────────────────────────────────────── */
void SparkBase::HandlePeriodicFrame(const can_frame& response)
{
  uint32_t receivedArbId = response.can_id & CAN_EFF_MASK;
  uint64_t rawValue      = 0;
...
// V25 updated dataframes
  if (receivedArbId == CreateArbId(APICommand::Period0)) {
    period0_.dutyCycle = int16_t(rawValue & 0xFFFF) / 32442.0f;
    ...
    period0_.temperature = (rawValue >> 40) & 0xFF;
    period0_.voltage = ((rawValue >> 16) & 0xFFF) / 128.0f;
    period0_.current = ((rawValue >> 28) & 0xFFF) / 32.0f;
    ...
    std::memcpy(&period2_.velocity, &velocity, 4);
  ...
  } else if (receivedArbId == CreateArbId(APICommand::Period5)) {
    uint32_t velocity = rawValue & 0xFFFFFFFF;
    uint32_t position = (rawValue >> 32) & 0xFFFFFFFF;
    std::memcpy(&period5_.absoluteEncoderVelocity, &velocity, 4);
    std::memcpy(&period5_.absoluteEncoderPosition, &position, 4);
    period5_.timestamp = now;
  }
...
// Status //
void SparkBase::SetPeriodicStatuses(uint8_t enabled)
{
  SetParameter(Parameter::kForceEnableStatus2, PARAM_TYPE_BOOL, "Force Enable Status Frame", true);
  for(int j=(int)Parameter::kForceEnableStatus0;j<=(int)Parameter::kForceEnableStatus7;j++)
  {
    bool enable = (enabled >> j) & 1;
...
void SparkBase::SetPeriodicStatus5Period(uint16_t period)
{
  std::vector<uint8_t> data(2, 0x00);
  data[0] = static_cast<uint8_t>(period & 0xFF);
  data[1] = static_cast<uint8_t>((period >> 8) & 0xFF);
  SendCanFrame(APICommand::Period5, data);
}

// Period 0
float SparkBase::GetDutyCycle() const
{
  // This no work anymore in V25
  std::lock_guard<std::mutex> lock(mutex_);
...
{
  // This no work anymore in V25 yet
  std::lock_guard<std::mutex> lock(mutex_);
  return false;
}
...
{
  std::lock_guard<std::mutex> lock(mutex_);
  return period0_.temperature;
}

float SparkBase::GetVoltage() const
{
  std::lock_guard<std::mutex> lock(mutex_);
  return period0_.voltage;
}

float SparkBase::GetCurrent() const
{
  std::lock_guard<std::mutex> lock(mutex_);
  return period0_.current;
}
...
// This is flipped, we are getting position in place of velocity
float SparkBase::GetAbsoluteEncoderVelocity() const
{
  std::lock_guard<std::mutex> lock(mutex_);
  return period5_.absoluteEncoderVelocity;
}

float SparkBase::GetAbsoluteEncoderPosition() const
{
  std::lock_guard<std::mutex> lock(mutex_);
  return period5_.absoluteEncoderPosition;
}
...
uint8_t SparkBase::GetIdleModeRaw()
{
